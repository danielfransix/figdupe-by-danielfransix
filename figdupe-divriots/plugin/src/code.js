...
async function applyNodeProps(node, props) {
  // Load font before any text operation — fall back to Inter if the font is unavailable
  if (node.type === 'TEXT') {
    const requested = props.fontName || { family: 'Inter', style: 'Regular' };
    const fallbacks = [
      requested,
      { family: requested.family, style: 'Regular' },
      { family: 'Inter', style: requested.style },
    ...
    if (!loaded) console.warn(`[figdupe] Could not load any font for "${requested.family} ${requested.style}" — text node may be empty`);
  }
